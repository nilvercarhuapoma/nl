...
void Representante::ingresar_representante(vector<Representante> &representante)
{
    ...
    // Variable para rastrear si se ha mostrado el mensaje de error
 bool errorMostrado = false;
    ...
    cout << "Ingrese una ubicacion georreferenciada:";
...
void Madera::ingresar_madera(vector<Madera> &datos_madera)
{
    ...
    do {
        ...
        if (!(cin >> information) || cin.peek() != '\n' || information <= 0) {
            cout << "Datos incorrectos. Intentalo de nuevo." << endl;
            cin.clear();
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
        } else {
            break;
    ...
    for (int i = 0; i < information; ++i)
    {
        ...
        cin >> tree.cantidad;
        cin.ignore();
        cout << "Ingrese el tamanio de la zona maderable (hectareas): ";
        cin >> tree.superficie;
        cin.ignore();
...
void Destino::Ingresar_datosdestino(vector<Destino> &dest, vector<Conductor> &conduct)
{
    ...
    cout << "Total de costo:" << destino.conductor.suma_ventas(conduct)<<endl;
